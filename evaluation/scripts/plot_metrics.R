...
ggplot(data_all %>% inner_join(samples) %>% filter(class %in% c("Total", "INS", "DEL")), aes(x=construct, cactus, color=strain, pch=clade)) +
  geom_point() +
...
ggsave(args[2], device = "png", width = 6, height=6)
